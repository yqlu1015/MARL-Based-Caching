...
class Request(object):
    # request of a DNN is represented by an type and its position
...
def generate_requests(num_users=10, num_types=5, num_low=80, num_high=120, zipf_param=0.6, orders=None) -> np.array:
    requests = np.zeros((num_users, num_types))
    if orders is None:
        orders = np.tile(np.arange(num_types), (num_users, 1))
    num_requests = np.random.randint(num_low, num_high, num_users)

    for i in range(num_users):
        request_indices = generate_zipf(num_types, zipf_param, num_requests[i], orders[i])
        # values, counts = np.unique(request_indices, return_counts=True)
        requests[i] = np.bincount(request_indices, minlength=num_types)
