...
from common.utils import to_tensor, index_to_one_hot, onehot_from_logits
...
class MFQS(Agent):
    ...
    def __init__(self, env, state_dim, action_dim, device='cpu',
        ...
        self.qnet = MeanQNet(self.state_dim, self.action_dim, self.actor_hidden_size, self.action_dim,
                             self.critic_output_act).to(device)
        self.qnet_target = MeanQNet(self.state_dim, self.action_dim, self.actor_hidden_size,
    ...
    def train(self):
        ...
        next_act = [onehot_from_logits(self.qnet(_next_obs, mean_actions_tensor[:, i]))
                    for i, _next_obs in enumerate(next_obs)]

        # compute Q(s_t, a) - the model computes Q(s_t), then we select the
        # columns of actions taken
        for i in range(self.n_agents):
            actions_index = actions_tensor[:, i].unsqueeze(1)
            current_q = self.qnet(obs[i], mean_actions_tensor[:, i]).gather(1, actions_index).squeeze(1)
            ...
            next_q = th.sum(self.qnet_target(next_obs[i], next_mean_action_tensor) * next_act[i], dim=1).detach()
