...
class MFAC(Agent):
    ...
    def __init__(self, env, state_dim, action_dim, device='cpu',
        ...
        self.actor_optimizer = [Adam(self.actor[i].parameters(), lr=self.actor_lr)
                                for i in range(self.n_agents)]
        self.critic = [
            MeanCriticNet(self.state_dim, self.env.n_contents, self.critic_hidden_size, 1,
    ...
    def train(self):
        ...
        for i in range(self.n_agents):
            ...
            # actions_list = [int2binary(idx.item(), self.env.n_contents) for idx in actions_index]
            ...
            # next_actions_list = [int2binary(idx.item(), self.env.n_contents) for idx in next_actions_index]
    ...
    def mean_action(self, state, evaluation=False):
        ...
        mean_actions = np.zeros((self.n_agents, self.env.n_contents))
