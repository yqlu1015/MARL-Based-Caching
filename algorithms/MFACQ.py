...
from common.utils import to_tensor, entropy, index_to_one_hot, index_to_one_hot_tensor, onehot_from_logits, sample_from_logits
...
class MFACQ(Agent):
    ...
    def __init__(self, env, state_dim, action_dim, device='cpu',
        ...
        self.critic = [
            MeanCriticNet(self.global_state_dim, self.action_dim, self.critic_hidden_size, self.action_dim, 1,
    ...
    def train(self):
        ...
        for i in range(self.n_agents):
            current_q = self.critic[i](global_state_tensor[:, i], act[i], mean_actions_tensor[:, i]).squeeze(1)
            ...
            next_q = self.critic_target[i](next_global_state_tensor[:, i], next_act[i], next_mean_action_tensor).squeeze(1).detach()
